...
async function migrate() {
    ...
    try {
        ...
        const cols = await q(`PRAGMA table_info(test)`);
        ...
        await add(
            "test_id",
            `ALTER TABLE test ADD COLUMN test_id TEXT`
        );

        await add(
            "is_test",
            `ALTER TABLE test ADD COLUMN is_test INTEGER DEFAULT 0`
        );

        await add(
            "test_status",
            `ALTER TABLE test ADD COLUMN test_status TEXT`
        );

        await add(
            "test_expected",
            `ALTER TABLE test ADD COLUMN test_expected TEXT`
        );

        await add(
            "test_user_id",
            `ALTER TABLE test ADD COLUMN test_user_id TEXT`
        );

        await add(
            "test_created_by",
            `ALTER TABLE test ADD COLUMN test_created_by TEXT`
        );
...
async function initSchema() {
    ...
    await db.batch([
        ...
        `CREATE INDEX IF NOT EXISTS idx_test_device    ON test(device_id)`,
        `CREATE INDEX IF NOT EXISTS idx_test_ts        ON test(timestamp)`,
...
app.post("/app/register", async (req, res) => {
    try {
        ...
        await run("INSERT INTO app_users (id,username,password_hash,created_at) VALUES (?,?,?,?)", [id, username.trim(), hash, Date.now()]);
        const appUser = await one("SELECT * FROM app_users WHERE id = ?", [id]);
        const token   = signAppToken(appUser);
        await run("INSERT INTO app_sessions (token,user_id,created_at,expires_at,revoked) VALUES (?,?,?,?,0)", [token, id, Date.now(), Date.now() + 30*24*3600000]);
...
app.post("/app/login", async (req, res) => {
    try {
        ...
        const token = signAppToken(appUser);
        await run("INSERT INTO app_sessions (token,user_id,created_at,expires_at,revoked) VALUES (?,?,?,?,0)", [token, appUser.id, Date.now(), Date.now() + 30*24*3600000]);
    ...
    try {
        ...
        const result  = await Promise.all(devices.map(async d => {
            ...
            return {
                ...
                nickname: d.nickname || d.device_id,
                online:   !!online,
                model:    d.model || "unknown",
                sims:     sims.map(s => ({ simId: s.sim_id, label: s.label, candidate: s.candidate }))
            };
        }));
...
app.patch("/app/device/:deviceId/rename", requireAppAuth, async (req, res) => {
    try {
        ...
        const device = await one("SELECT * FROM devices WHERE device_id = ?", [deviceId]);
        if (!device || device.app_user_id !== req.appUser.id) return res.status(403).json({ error: "Non autorizzato" });
        await run("UPDATE devices SET nickname = ? WHERE device_id = ?", [nickname.trim(), deviceId]);
        io.emit("device_renamed", { deviceId, nickname: nickname.trim() });
        res.json({ ok: true, nickname: nickname.trim() });
    } catch(e) { res.status(500).json({ error: e.message }); }
});

app.get("/app/test", requireAppAuth, async (req, res) => {
    try {
        const rows = await q("SELECT * FROM test WHERE is_test = 1 AND test_user_id = ? ORDER BY timestamp DESC LIMIT 500", [req.appUser.id]);
        res.json(rows);
    } catch(e) { res.status(500).json({ error: e.message }); }
});
...
io.use((socket, next) => {
    ...
    if (deviceKey && timingSafe(deviceKey, DEVICE_API_KEY)) {
        ...
        if (appToken) {
            try { const p = jwt.verify(appToken, JWT_SECRET); if (p.type === "app") socket.appUserId = p.sub; } catch {}
        }
        return next();
    }
    ...
    socket.on("register_device", async (data) => {
        ...
        try {
            ...
            devicesOnline[socket.id] = {
                deviceId: data.deviceId, model: data.model || "unknown",
                sims: [], connectedAt: Date.now(), appUserId: socket.appUserId || null
            };

            // Upsert device
            const existing = await one("SELECT * FROM devices WHERE device_id = ?", [data.deviceId]);
            if (!existing) {
                await run("INSERT INTO devices (device_id,nickname,model,app_user_id,created_at) VALUES (?,?,?,?,?)",
                    [data.deviceId, null, data.model || "unknown", socket.appUserId || null, Date.now()]);
            } else {
                await run("UPDATE devices SET model = ?, app_user_id = COALESCE(?, app_user_id) WHERE device_id = ?",
                    [data.model || existing.model, socket.appUserId || null, data.deviceId]);
            }
            ...
            for (const sim of (data.sims || [])) {
                ...
                if (!existingSim) {
                    await run("INSERT INTO sims (id,device_id,sim_id,label,candidate,last_seen) VALUES (?,?,?,?,?,?)",
                        [crypto.randomUUID(), data.deviceId, sim.simId, null, sim.number?.trim() || null, Date.now()]);
                } else if (sim.number?.trim()) {
                    await run("UPDATE sims SET candidate = ?, last_seen = ? WHERE device_id = ? AND sim_id = ?",
                        [sim.number.trim(), Date.now(), data.deviceId, sim.simId]);
                }
...
// test
// ────────────────────────────────────────────
app.post("/test", authDevice, async (req, res) => {
    try {
        const test = req.body;

        if (!test?.deviceId || !test?.simId) {
        ...
        console.log("📩 test ricevuto:", {
            deviceId: test.deviceId,
            simId: test.simId,
            message: test.message
        });

        console.log("🧪 Pending tests:", pending.map(t => ({
            id: t.id,
            device: t.device_id,
            sim: t.sim_id,
            expected: t.expected,
        ...
        const matchedTest = pending.find(test =>
            test.device_id === test.deviceId &&
            test.sim_id === test.simId &&
            msg.includes(
                (test.expected || "").toLowerCase()
            )
        );

        console.log("🎯 Matched test:", matchedTest || "NONE");
        ...
        const dev = Object.values(devicesOnline)
            .find(d => d.deviceId === test.deviceId);

        if (
            dev &&
            !dev.sims.find(s => s.simId === test.simId)
        ) {
            dev.sims.push({
                simId: test.simId
            });
...
app.get("/test", requireAuth, async (req, res) => {
    try {
        ...
        // USER vede SOLO i suoi test-test
        const rows = await q(`
            SELECT *
            FROM test
            WHERE
...
app.get("/stats", requireAuth, async (req, res) => {
    try {
        ...
        const avgLatency = passTime.length > 0
            ? Math.round(passTime.reduce((a,t) => a + (t.completed_at - t.created_at), 0) / passTime.length)
            : null;
        ...
        res.json({
            summary: { total24, pass24, fail24, deliveryRate, avgLatency, totalSims: Number(totalSims[0].c), onlineDevices: Object.keys(devicesOnline).length },
            byCountry, hourly
        });
    } catch(e) { res.status(500).json({ error: e.message }); }
});
...
async function startScheduleJob(s) {
    ...
    scheduleJobs[s.id] = setInterval(run_, s.interval_ms);
}

app.get("/schedules", requireAuth, async (req, res) => {
    try { res.json(await q("SELECT * FROM schedules")); }
    catch(e) { res.status(500).json({ error: e.message }); }
});
...
setInterval(async () => {
    ...
    } catch(e) { console.error("Timeout job error:", e.message); }
}, 5000);

setInterval(async () => {
    try { await run("DELETE FROM sessions WHERE expires_at < ? OR revoked = 1", [Date.now()]); }
    catch(e) { console.error("Clean sessions error:", e.message); }
}, 3600000);
